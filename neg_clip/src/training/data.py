...
class CsvDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        chosen_caption = self.hard_captions[idx]
        hard_captions = tokenize([str(chosen_caption)])[0]
        logging.debug(
            f"before tokenizer text: {str(self.captions[idx])}"
            # f"after tokenzier text: {texts}"
            f"---------"
            f"before tokenizer hard caption: {str(chosen_caption)}"
            # f"after tokenizer hard captions: {hard_captions}"
            )
        ...
        # logging.debug(f"get hard negative caption: {idx}:-----{hard_captions}")
