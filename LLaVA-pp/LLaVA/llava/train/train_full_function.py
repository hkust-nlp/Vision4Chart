...
from llava.mm_utils import tokenizer_image_token, expand2square, process_anyres_image
...
class DataArguments:
    ...
    translation_ratio: float = 1/4
    image_folder: Optional[str] = field(default=None)
    image_aspect_ratio: str = 'square'
    image_grid_pinpoints: Optional[str] = field(default=None)
    raw_image_folder: Optional[str] = field(default=None)
...
def check_grad(module):
    # check if all/some/none of the parameters require grad in the module
    require_grad_info = [p.requires_grad for p in module.parameters()]
    if all(require_grad_info):
        return "all"
    elif any(require_grad_info):
        return "some"
    return "none"
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        if getattr(trainer.args, "tune_vision_tower", False):
            ...
            trainer.model.config.mm_vision_tower_lr = trainer.args.vision_tower_lr
...
def apply_random_augmentations(image, background_color, data_args):
    """
    Apply random rotation and translation augmentations to an image while keeping all original content.
    The rotation angle and translation are picked randomly.

    :param image_path: Path to the image file.
    :return: PIL Image with applied augmentations.
    """
    # Load the image

    # Random rotation angle between 0 and 359 degrees
    rotation_angle = random.randint(0, 359)
    translation_ratio = data_args.translation_ratio
    # Calculate maximum translation based on 1/4 of the longest side of the image
    max_translation = int(max(image.width, image.height)*translation_ratio)
    # max_translation = max(image.width, image.height) // 2
    translation = (random.randint(-max_translation, max_translation),
                   random.randint(-max_translation, max_translation))

    # Apply rotation with expansion
    rotated_image = image.rotate(rotation_angle, expand=True)

    # Calculate new image size after translation
    new_width = rotated_image.width + abs(translation[0])
    new_height = rotated_image.height + abs(translation[1])

    # Create a new image with the new size
    new_image = Image.new("RGB", (new_width, new_height), background_color)

    # Paste the rotated image onto the new image with an offset
    new_image.paste(rotated_image, (max(translation[0], 0), max(translation[1], 0)))
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if 'annotation_type' in self.list_data_dict[i]:
                if self.list_data_dict[i]['annotation_type'] == 'unlabel':
                    image_folder = self.data_args.raw_image_folder
                else:
            ...
            background_color = tuple(int(x*255) for x in processor.image_mean) if  not self.data_args.white_bg else (255, 255, 255)
            image_grid_pinpoints = self.data_args.image_grid_pinpoints
            if self.data_args.use_img_aug:
                image = apply_random_augmentations(image, background_color, self.data_args)
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                image = process_anyres_image(image, processor, background_color, image_grid_pinpoints)
            else:
                image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
...
def unlock_vit(training_args, model_args, vision_tower):
    ...
    training_args.vision_tower_lr = lr_of_vit
    rank0_print(
...
def train(attn_implementation=None):
    ...
    if model_args.freeze_backbone:
        # this freeze lm part, as it goes before init vision tower and mm projector
    ...
    if model_args.vision_tower is not None:
        ...
        # if data_args.image_grid_pinpoints is None and model.config.image_grid_pinpoints is not None:
        #     # read the image grid pinpoints from the model config and assign it to data_args
        #     # so that they can be used later
        #     data_args.image_grid_pinpoints = model.config.image_grid_pinpoints
        # elif data_args.image_grid_pinpoints is not None:
        #     # reset the image grid pinpoints as we assign new ones
        #     model.config.image_grid_pinpoints = data_args.image_grid_pinpoints
        if data_args.image_grid_pinpoints is None:
            if hasattr(model.config, 'image_grid_pinpoints') and model.config.image_grid_pinpoints is not None:
                # read the image grid pinpoints from the model config and assign it to data_args
                # so that they can be used later
                data_args.image_grid_pinpoints = model.config.image_grid_pinpoints
        elif data_args.image_grid_pinpoints is not None:
            ...
            if hasattr(model.config, 'image_grid_pinpoints'):
                ...
                # If model.config does not have this attribute, handle this case (e.g., add it)
                raise AttributeError("The model config does not have the attribute 'image_grid_pinpoints'.")

        
        model.config.image_aspect_ratio = data_args.image_aspect_ratio
        model.config.white_bg = data_args.white_bg
        ...
        """
        add the function to unlock the vision tower
        ref @ https://github.com/InternLM/InternLM-XComposer/blob/9ac26d911fe2075f96e9b5fe8e8cddd76c4db076/projects/ShareGPT4V/share4v/train/train.py#L969
        """
        ...
        # check if backbone is freezed, i.e. grad is False
        
        # find all attr in model.model that is a nn.Module
        for name, module in model.model.named_children():
            # print with color yellow
            grad_info = check_grad(module)
            rank0_print(f"\033[93m{name} | {grad_info}\033[0m")
        grad_info_lm_head = check_grad(model.lm_head)
        rank0_print(f"\033[93m{'lm_head'} | {grad_info_lm_head}\033[0m")
