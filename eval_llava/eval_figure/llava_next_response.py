...
if __name__ == '__main__':
    ...
    model = LlavaNextForConditionalGeneration.from_pretrained("/project/deemreason/junteng/Vision4Math/data/llava-v1.6-mistral-7b-hf", torch_dtype=torch.float16, low_cpu_mem_usage=True)
    model = model.to(device=DEVICE)

    processor = LlavaNextProcessor.from_pretrained("/project/deemreason/junteng/Vision4Math/data/llava-v1.6-mistral-7b-hf")
    model.eval()
    ...
    for pid, _ in enumerate(tqdm(query_list)):
        ...
        try:
            ...
            response = generated_texts[0].split('[/INST]')[-1].strip()
